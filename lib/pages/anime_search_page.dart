...
class _AnimeSearchPageState extends State<AnimeSearchPage> {
  ...
  void _doSearch() async {
    ...
    final list = await _service.searchSubject(_controller.text, type: 2, page: 1);
  ...
  Future<void> _loadNextPage() async {
    ...
    _page++;
    final newResults = await _service.searchSubject(_controller.text, type: 2, page: _page);
        ...
        title: Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: const Icon(Icons.movie_filter, color: Colors.grey),
              contentPadding: const EdgeInsets.symmetric(vertical: 0),
            ),
          ),
        ),
        actions: [
          TextButton(onPressed: _doSearch, child: const Text("搜番", style: TextStyle(color: themeColor, fontWeight: FontWeight.bold, fontSize: 16)))
        ],
      ),
      body: _isLoading 
        ? const Center(child: CircularProgressIndicator(color: themeColor))
        : !_hasSearched
          ? Center(child: Text("输入番剧名开始搜索", style: TextStyle(color: Colors.grey[400])))
          : _results.isEmpty
            ? Center(child: Text("没有找到相关番剧", style: TextStyle(color: Colors.grey[400])))
            : ListView.separated(
